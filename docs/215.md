...
| **9。** | OpenStack 使用 MongoDB、Cassandra 或 Couchbase 作为非关系数据库。 | 对于非关系数据库，AWS 使用 EMR(弹性MapReduce)。 |
...
| **14。** | 云高仪-基于遥测技术的计费、资源跟踪等。 | AWS使用和计费报告。 |
...
撒哈拉是一种数据处理服务，也称为弹性MapReduce，它提供了一个平台，使用处理框架(如 Spark 和 Hadoop)来供应和管理实例集群。
